...
export const TaskList = (props: Props) => {
  ...
  const getHeader = () => {
    if (props.compact) return <></>;
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={3}>{t("tasks.taskList.associateW")}</Grid>
        <Grid item xs={3}>{t("tasks.taskList.assignTo")}</Grid>
      </Grid>
    </div>)
  }
  ...
  const getAssignedToMyGroups = () => {
    if (groupMembers?.length > 0) {
      ...
      if (assignedToMyGroups.length === 0) return <></>
        ...
        <h4>{t("tasks.taskList.assignGroup")}</h4>
        {getHeader()}
  ...
  const getAssignedToMe = () => {
    ...
    if (assignedToMe.length === 0) return <></>
      ...
      <h4>{t("tasks.taskList.assignMe")}</h4>
      {getHeader()}
  ...
  return (<>
    ...
    <DisplayBox headerIcon="list_alt" headerText={t("tasks.taskList.tasks")} editContent={editContent} help="chums/tasks">
      {getAssignedToMe()}
