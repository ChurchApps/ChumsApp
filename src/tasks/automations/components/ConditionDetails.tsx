...
import {
  Add as AddIcon,
  Edit as EditIcon,
  Done as ConditionIcon,
  AccountTree as ConjunctionIcon,
  Rule as RuleIcon
  ...
  const getConditionMenu = () => (
    ...
    >
      <MenuItem onClick={() => { 
        ...
        <ListItemIcon><ConjunctionIcon /></ListItemIcon>
    ...
    conditions?.forEach(condition => {
      result.push(
        ...
        >
          ...
          <IconButton 
            size="small" 
            onClick={() => props.setEditCondition(condition)} 
            data-testid={`edit-condition-button-${condition.id}`} 
        ...
        <Box key={conjunction.id}>
          ...
          >
            ...
            <Stack direction="row" spacing={0.5}>
              <IconButton 
                size="small" 
                onClick={(e) => { setMenuAnchor(e.currentTarget); setParentId(conjunction.id); }} 
              ...
              <IconButton 
                size="small" 
                onClick={() => props.setEditConjunction(conjunction)} 
                data-testid={`edit-conjunction-button-${conjunction.id}`} 
                aria-label="Edit conjunction"
              >
                <EditIcon fontSize="small" />
              </IconButton>
            </Stack>
