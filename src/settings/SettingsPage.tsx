...
import { UserHelper, BigLinkButton, Permissions, ChurchAppInterface, ArrayHelper } from "./components";
...
export const SettingsPage = () => {
  ...
  const getLinks = () => {
    const accessMangementApp: ChurchAppInterface = ArrayHelper.getOne(churchApps, "appName", "AccessManagement");
    const returnUrl = `/churches/${accessMangementApp?.churchId.toString()}/manage`;
    let result = [];
    ...
    if (accessMangementApp) result.push(<BigLinkButton key="managePermissions" outsideLink={true} href={UserHelper.goToAccountsApp(returnUrl)} icon="fas fa-lock" text="Manage Permissions" />);
