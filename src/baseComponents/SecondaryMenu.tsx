...
export const SecondaryMenu= (props:Props) => {
  ...
  const getItems = () => {
    ...
    props.menuItems.forEach(item => {
      if (item.label === props.label) result.push(<Chip onClick={() => navigate(item.url)} label={item.label} component="a" variant="filled" clickable style={{marginLeft:10, marginRight:10, backgroundColor:"#114A99", color:"#FFF", fontSize:16}} />);
