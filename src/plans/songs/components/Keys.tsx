...
import { ApiHelper, ArrayHelper, type LinkInterface, Locale, DisplayBox } from "@churchapps/apphelper";
import { 
  Alert, 
  Box, 
  Button, 
...
export const Keys = memo((props: Props) => {
  ...
  if (editKey) {
    return (
      <KeyEdit 
        ...
        onSave={() => { setEditKey(null); loadData(); }} 
  ...
  return (
    <>
                ...
                label={
                  <Stack direction="row" spacing={1} alignItems="center">
                    ...
                    <Typography variant="body2">
                      {Locale.label("songs.keys.add") || "Add Key"}
                    </Typography>
                  </Stack>
                }
              />
      ...
      >
        <MenuItem onClick={() => { handleClose(); setShowImport(true); }}>
      ...
      {editLink && (
        <LinkEdit 
          link={editLink} 
          onSave={() => { setEditLink(null); loadLinks(); }} 
      ...
      {showImport && (
        <PraiseChartsProducts 
          ...
          onHide={() => { setShowImport(false); loadPraiseCharts(); }} 
        />
      )}
    </>
  );
