...
export const Arrangements = (props: Props) => {
  ...
  const getArrangements = () => {
    ...
    if (!arrangements) return result;
    ...
    else if (arrangements.length === 1) result.push(<Arrangement arrangement={arrangements[0]} originalKey={props.songDetail?.keySignature} />);
    else {
      ...
      arrangements.forEach((arrangement: any) => {
        result.push(<Accordion key={arrangement.id}>
          ...
          <AccordionDetails>
            <Arrangement arrangement={arrangement} originalKey={props.songDetail?.keySignature} />
          </AccordionDetails>
        </Accordion>);
      });
