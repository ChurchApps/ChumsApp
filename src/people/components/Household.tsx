...
export const Household: React.FC<Props> = (props) => {
    ...
    const getRows = () => {
        ...
        if (members !== null) {
            for (let i = 0; i < members.length; i++) {
                ...
                rows.push(
                    <tr key={m.id}>
                        <td><img src={photo} alt="avatar"/></td>
    ...
    if (mode === "display") {
        return (
            <DisplayBox id="householdBox" headerIcon="fas fa-users" headerText={(household?.name || "") + " Household"}
                        editFunction={getEditFunction()} ariaLabel="editHousehold">
                {getTable()}
            </DisplayBox>
        );
