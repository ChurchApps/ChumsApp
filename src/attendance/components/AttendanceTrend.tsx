...
export const AttendanceTrend = () => {
    ...
    const getFilterOptions = (data: any[]) => {
        var result: ReportFilterOptionInterface[] = [];
        data?.forEach(d => { result.push({ keyName: d.id.toString(), value: d.id.toString(), label: d.name }) });
        return result;
    }

    const getCampusOptions = () => { return getFilterOptions(campuses) };
    const getServiceOptions = () => { return getFilterOptions(services) };
    const getServiceTimeOptions = () => { return getFilterOptions(serviceTimes) };
    const getGroupOptions = () => { return getFilterOptions(groups) };
    const getCategoryOptions = () => {
        var result: ReportFilterOptionInterface[] = [{ keyName: "Any", value: "", label: "Any" }];
    ...
    const loadReport = async (filter: ReportFilterInterface) => {
        ...
        var url = `/attendancerecords/trend?campusId=${campusId}&serviceId=${serviceId}&serviceTimeId=${serviceTimeId}&groupId=${groupId}`;
