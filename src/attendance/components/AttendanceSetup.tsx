...
import { useMountedState, type AttendanceInterface, type CampusInterface, type ServiceInterface, type ServiceTimeInterface, type GroupServiceTimeInterface, type GroupInterface, ApiHelper, ArrayHelper, Loading, Locale } from "@churchapps/apphelper";
...
export const AttendanceSetup = memo(() => {
    ...
    return [
      <TableRow key="header">
        <TableCell sx={{ fontWeight: 600, color: '#666' }}>
          {Locale.label("attendance.attendancePage.campus")}
        </TableCell>
        <TableCell sx={{ fontWeight: 600, color: '#666' }}>
          {Locale.label("attendance.attendancePage.service")}
        </TableCell>
        <TableCell sx={{ fontWeight: 600, color: '#666' }}>
          {Locale.label("attendance.attendancePage.time")}
        </TableCell>
        <TableCell sx={{ fontWeight: 600, color: '#666' }}>
          {Locale.label("attendance.attendancePage.category")}
        </TableCell>
        <TableCell sx={{ fontWeight: 600, color: '#666' }}>
          {Locale.label("attendance.attendancePage.group")}
        </TableCell>
      </TableRow>
    ];
  }, [attendance.length]);
  ...
  const getRows = useCallback(() => {
    ...
    const getRow = (campus: CampusInterface, service: ServiceInterface, serviceTime: ServiceTimeInterface, group: GroupInterface, key: string) => {
      const campusHtml = (campus === undefined || campus?.name === lastCampus) ? <></> : (
      ...
      const serviceHtml = (service === undefined || service?.name === lastService) ? <></> : (
        <Stack direction="row" spacing={1} alignItems="center">
      ...
      const serviceTimeHtml = (serviceTime === undefined || serviceTime?.name === lastServiceTime) ? <></> : (
        <Stack direction="row" spacing={1} alignItems="center">
      ...
      const categoryHtml = (group === undefined || group?.categoryName === lastCategory) ? <></> : (
        <Stack direction="row" spacing={1} alignItems="center">
      ...
      const groupHtml = (group === undefined) ? <></> : (
        <Stack direction="row" spacing={1} alignItems="center">
